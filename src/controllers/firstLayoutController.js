/**
 * Adds a new `firstLayout()` lifecycle method to its host,
 ...
 */
export class FirstLayoutController {
  host;
  _requestId;
  _wasNotCalledYet;
  ...
  firstLayout() {
    ...
    this.host.firstLayout?.call(this.host);
  }

  hostUpdated() {
    // `hostFirstUpdated()` does not exist in Lit ReactiveController interface;
    // we are simulating it, to ensure `firstLayout()` gets called exactly once
    ...
    if(this._wasNotCalledYet
      ...
      // before next animation frame (could happen when page is opened in an unvisible tab)
       && this._requestId === null) {
      this._requestId = window.requestAnimationFrame(this.firstLayout.bind(this));
    }
  }
